...
test_transform = None
train_dataset = RGBTrainDataloader(train_path, mask_path,
                                   transform=train_transform, concat=concat_flag,
                                   data_name=data_name,
                                   rgb_input=mode[output_mode][0], 
                                   rgb_label=mode[output_mode][1], load_mode=load_mode[data_name])
                                   
train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                               shuffle=True, num_workers=8)
test_dataset = RGBTrainDataloader(test_path, eval_mask_path,
