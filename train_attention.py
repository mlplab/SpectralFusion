...
from data_loader import PatchMaskDataset, SpectralFusionDataset, RGBPreTrainDataloader
...
train_dataset = PatchMaskDataset(train_path, mask_path, transform=train_transform, 
                                 concat=concat_flag, load_mode=load_mode[data_name])
train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, 
                                               shuffle=True, num_workers=8)
test_transform = None
test_dataset = PatchMaskDataset(test_path, eval_mask_path, transform=test_transform, 
                                concat=concat_flag, load_mode=load_mode[data_name])
test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, 
                                              shuffle=False, num_workers=8)


model = SSAttention(input_ch, 31, feature_num=31, layer_num=block_num).to(device)
